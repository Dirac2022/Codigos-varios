...
map<int, set<int>> mode(vector<int>& X, int n);
...
int main()
{
    ...
    printArrayX(array5);
    map<int, set<int>> modas = mode(array5, 200);

    for(const auto& pair : modas) {
        int moda = pair.first;
        set<int> modeValues = pair.second;
        if (modeValues.size() == 1) {
            cout << "La moda de frecuencia " << moda << " es: " << *modeValues.begin() << endl;
        }
        else {
            cout << "Las modas de frecuencia " << moda << " son: " << endl;
...
map<int, set<int>> mode(vector<int>& X, int n)
{
    ...
    for(int i = 0; i < n; i ++) {
        ...
        if (frequency > maxFrequency) {
            ...
            modes.clear();
            oe ++;
            modes.insert(make_pair(frequency, set<int>{X[i]}));
            oe ++;
        }

        else if (frequency == maxFrequency) {
            oe ++;
            modes[frequency].insert(X[i]);
            oe ++;
        }
    }
